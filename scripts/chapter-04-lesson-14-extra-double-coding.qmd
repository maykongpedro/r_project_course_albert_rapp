---
title: "lesson-14-extra-double-coding"
format: html
editor: visual
editor_options: 
...
O objetivo desse script é formatar o gráfico do capítulo 4 com shapes representando cada grupo (fêmeas e machos). Levantei essa dúvida no curso e o professor me enviou um material gratuito que ele colocou no youtube, no seguinte link: [aqui](https://www.youtube.com/watch?v=X1gkw--k5Tg).

## Requisitos:

-   Necessário instalar a fonte de texto do site de ícones [Font Awesome](https://fontawesome.com/download).

## Etapas

1.  Instalar a font necessária
2.  Buscar o nome dos ícones no site de fontes.
3.  Criar um vetor de símbolos com o nome dos ícones e nomear os dados do vetor com os nomes do grupo que será plotado.
4.  Adicionar uma coluna na base para classificar qual o símbolo que vai ser recebido.
5.  Substituir o geom_point por geom_text e utilizar a coluna de símbolo criada.
6. Modificar o argumento `fill` do ggplot2::ggplot2 para `color` pois o geom_text não tem fill.
7. Ajustar o `ggplot2::scale_fill_manual` para `scale_color_manual`
8. Especificar no argumento `family` do `geom_text` a família referente a fonte de texto *Font Awesome 6 Free*
9. Ajustar o texto no subtítulo para representar os ícone usados
